...
def process_quake(quake, current_time):
    """
    ...
    # Calculate if the earthquake happened within the last minute
    last_hour = 1 if (current_time - time_dt) <= timedelta(minutes=1) else 0
...
def update_dataframe(new_data, df, state=None):
    ...
    for quake in new_data:
        ...
        quake_id = quake.get("id")
        processed_quake = process_quake(quake, current_time)
    ...
    df["last_hour"] = df["time"].apply(
        lambda x: 1 if (current_time - x) <= timedelta(minutes=60) else 0
    )
