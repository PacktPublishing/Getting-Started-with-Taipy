...
with tgb.Page() as cache_analyze_page:
    ...
    with tgb.layout("1 1 1"):
        ...
        with tgb.part("card"):
            ...
            tgb.text(
                "### {percentage_no_tip} %", mode="md", class_name="color-secondary"
            )
    with tgb.layout("1 1"):
        tgb.chart(
            "{df_weekend}", type="bar", x="is_weekend", y="average_tip", rebuild=True
        )
        tgb.chart("{df_night}", type="bar", x="is_night", y="average_tip", rebuild=True)
        tgb.chart(
            "{df_hour}", type="bar", x="hour_of_day", y="average_tip", rebuild=True
        )
        tgb.chart(
            "{df_from_airport}",
            type="bar",
            x="airport_fee_binary",
            y="average_tip",
            rebuild=True,
        )
